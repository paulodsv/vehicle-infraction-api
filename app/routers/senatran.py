...
def consult_infractions(data: SenatranInfractionQuery, 
                        ...
                        service: SenatranService = Depends(get_senatran_service)) -> APIResponse[list[InfractionResponse]]:
    infractions = service.consult_infractions_service(data, current_user.id)
...
@senatran_router.post("/consult-fleet", 
                      ...
                      summary="Consulta as infrações da frota", 
                      description="Consulta e grava no banco as infrações de todos os veículos cadastrados no sistema que estão ativos")
def consult_fleet_infractions(current_user: User = Depends(get_current_user), 
                              service: SenatranService = Depends(get_senatran_service)) -> APIResponse:
...
@senatran_router.post("/fetch-details", status_code=200, 
                      ...
                      responses={
                          200: {"description": "Infrações atualizadas com sucesso"},
...
def fetch_infraction_details(get_current_user: User = Depends(get_current_user), 
    ...
    return success_response(count, "Infrações atualizadas com sucesso")